...
exports.up = async (knex) => {
  ...
  const sqlStatement = 'jsonb_set(last_status_response::jsonb, \'{obj2s}\', '
    + 'last_status_response::jsonb->\'seurat\', true) '
    + '- \'seurat\'';
  await knex('experiment_execution')
    ...
    .update({
      ...
      // with the value from `seurat` and remove the old `seurat` key.
      last_status_response: knex.raw(sqlStatement),
    });
};
